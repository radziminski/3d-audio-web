...
export default function TestIdResultsPage({
  guesses,
  guessesQuality,
  ...
  const filteredGuesses = guesses.filter((guess) => guess.testId === testId);
  const filteredGuessesQuality = guessesQuality.filter(
    (guess) => guess.testId === testId
  );

  if (!testId) return <h1>No test id!</h1>;
        ...
        <Layout withScroll>
          <div className={classes.content}>
            <h2 style={{ margin: 0, paddingTop: '32px' }}>
              Test Result for Test ID: {testId}
            </h2>
            <h3>UserId: {filteredGuesses[0].userId}) </h3>
